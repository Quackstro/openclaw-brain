/**
 * Brain 2.0 — LanceDB store manager.
 *
 * Manages all 11 tables: inbox, 8 main buckets, needs_review, audit_trail.
 * Follows the lazy-init pattern from doc-RAG.
 *
 * Store location: ~/.openclaw/brain/ (separate from docrag)
 */
...
export class BrainStore {
  ...
  async ensureInitialized(): Promise<void> {
    if (this.db && this.tables.size === ALL_TABLES.length) return;
  ...
  private async doInitialize(): Promise<void> {
    ...
    for (const tableName of ALL_TABLES) {
      if (existingTables.includes(tableName)) {
        ...
        const seed = schemaSeed(tableName, this.vectorDim);
   ...
   */
  async get(tableName: TableName, id: string): Promise<Record<string, unknown> | null> {
    const table = await this.getTable(tableName);
   ...
   */
  async list(tableName: TableName, limit?: number): Promise<Record<string, unknown>[]> {
    const table = await this.getTable(tableName);
