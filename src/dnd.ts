...
let _dndStatePath: string | null = null;

/** Set the DND state file path. Called during plugin init. */
export function setDndStatePath(storagePath: string): void {
  _dndStatePath = path.join(storagePath, "dnd-state.json");
}
...
export async function saveDndState(state: DndState): Promise<void> {
  await fs.mkdir(path.dirname(getDndStatePath()), { recursive: true });
  await fs.writeFile(getDndStatePath(), JSON.stringify(state, null, 2));
}
