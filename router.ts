/**
 * Brain 2.0 — Confidence check + bucket routing.
 *
...
/** Default confidence threshold (from design doc Section 5.6). */
export const DEFAULT_CONFIDENCE_THRESHOLD = 0.80;

/** Cosine similarity threshold for auto-merge (design doc Section 5.6). */
export const DEDUP_SIMILARITY_THRESHOLD = 0.92;

/** Max input length before suggesting split (design doc Section 5.6). */
...
export function checkConfidence(
  ...
  const tableName = bucketToTable(bucket);
  if (!tableName) {
    return {
      routable: false,
      bucket: null,
...
export function buildBucketRecord(
  ...
  switch (bucket) {
    ...
    default:
      throw new Error(`Cannot build record for bucket: ${bucket}`);
  }
}
...
function inferAdminCategory(
  ...
  if (text.includes("appointment") || text.includes("dentist") || text.includes("meeting"))
    return "appointment";
  if (text.includes("errand") || text.includes("pick up") || text.includes("buy"))
    return "errand";
  if (text.includes("bill") || text.includes("pay")) return "bill";
  if (text.includes("ship") || text.includes("move") || text.includes("travel"))
    return "logistics";
  return "other";
}

function inferGoalTimeframe(
  c: ClassificationResult,
): "short-term" | "medium-term" | "long-term" {
  const text = `${c.title} ${c.summary}`.toLowerCase();
  if (text.includes("this week") || text.includes("today") || text.includes("tomorrow"))
    return "short-term";
  if (text.includes("this month") || text.includes("this quarter"))
    return "medium-term";
...
function inferHealthCategory(
  ...
  if (
    ...
    return "medical";
  if (text.includes("gym") || text.includes("workout") || text.includes("exercise") || text.includes("run"))
    return "fitness";
  if (text.includes("diet") || text.includes("nutrition") || text.includes("meal"))
    return "nutrition";
  if (text.includes("therapy") || text.includes("mental") || text.includes("stress") || text.includes("meditation"))
    return "mental";
  return "wellness";
}
...
function inferFinanceCategory(
  ...
  const text = `${c.title} ${c.summary}`.toLowerCase();
  if (text.includes("bill") || text.includes("pay") || text.includes("electricity") || text.includes("rent"))
    return "bill";
...
export async function mergeIntoExisting(
  ...
  const mergedActions = [
    ...new Set([...existingActions, ...classification.nextActions]),
  ];
...
export async function routeClassification(
  ...
  const check = checkConfidence(classification, threshold);
