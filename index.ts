...
const brainPlugin = {
  ...
  register(api: any) {
    ...
    // NOTE: Payment callbacks (brain:pay:approve/dismiss/edit) are handled
    // at the agent level via AGENTS.md instructions. OpenClaw's plugin system
    // doesn't support intercepting Telegram callback_data before it reaches
    // the agent session. The wallet-unlock file watcher (in the service below)
    // handles auto-retry for locked wallet scenarios.
