/**
 * Brain 2.0 — /drop command handler.
 *
 * Behavior:
...
import { routeActions, type ActionRouterConfig } from "../action-router.js";
...
export interface DropOptions {
  ...
  async?: boolean;
  /** Action router config — creates cron reminders for time-sensitive drops */
  actionRouterConfig?: ActionRouterConfig;
}
 ...
 * @param options - Optional drop options (classifier, threshold, async)
 */
  ...
  if (options?.classifierFn) {
    const classifyAndRoute = async () => {
      try {
        ...
        const { classification, tokensUsed } =
          await options.classifierFn!(textForClassification);
        ...
        // Action router: handle time-sensitive drops + intent-based actions
        if (options?.actionRouterConfig && options.actionRouterConfig.enabled !== false) {
          try {
            const actionResult = await routeActions(
              store,
              classification,
              textForClassification,
              inboxId,
              options.actionRouterConfig!,
              inputTag,
            );
            if (actionResult.action !== "no-action") {
              console.log(`[brain] Action router: ${actionResult.action} — ${actionResult.details}`);
            }
          } catch (err) {
